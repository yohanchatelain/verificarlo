...
BacktraceLine = namedtuple(
    'BacktraceLine', ['binary_path', 'function', 'address'])
...
def get_hash_backtrace_list(backtrace_list):
    backtrace_string = "".join(["".join(backtrace)
...
def parse_backtrace_line(line):
    try:
        ...
        print(f"Error while parsing backtrace line: {line}")
        print(e)
        exit(1)
...
def add_backtrace_mapping(backtrace_list_name, dirs_list):
    ...
    if backtrace_map_file == None:
        backtrace_map_file = open(
            backtrace_dirname + os.sep + backtrace_map_filename, "w")
...
def split_bt_list(bt_list, counter):
    ...
    for line in diff_lines:
        ...
        bt_list_splitted.append(
            list(map(lambda bt: bt[counter+1:], bt_list_filtered)))

    return list(filter(lambda bt: bt != [], bt_list_splitted))
...
def check_divergence(bt_list, counter):
    ...
    for i in range(minlen):
        line = list(map(lambda bt: bt[i], bt_list))
        ...
        if not is_good_line:
            ...
            bt_tree.extend(list(map(lambda bt: check_divergence(
                bt, counter+1), bt_list_splitted)))
            break
        counter_local += 1
        counter += 1
    if minlen != maxlen:
        ...
        bt_tree.extend(list(map(lambda bt: check_divergence(
            bt, counter+1), bt_list_splitted)))
    return list(filter(lambda bt: bt != [], bt_tree))
...
def pretty_printer_list(dirs_list):
    ...
    try:
        ...
        sorted_dirs_list_int = list(sorted(dirs_list_int, reverse=True))
...
def get_dir_list(args, getBacktraceDir=False):
    ...
    # dir_list = list(sorted(os.listdir("."), key=lambda d: int(d)
    #                        if d.isdigit() else d))
    dir_list_int = list(filter(lambda d: d.isdigit(), os.listdir(".")))
    dir_list = list(sorted(dir_list_int))
    filtered_dir_list = list(
        filter(lambda d: d not in ignored_dir_list, dir_list))
    dirs = list(filter(os.path.isdir, filtered_dir_list))

    def isfile(filename): return lambda d: os.path.isfile(d+os.sep+filename)

    def isnotfile(filename): return lambda d: not os.path.isfile(
        d+os.sep+filename)

    def isemptyfile(filename): return lambda d: os.path.getsize(
        d+os.sep+filename) == 0
    def isnotemptyfile(filename): return lambda d: os.path.getsize(
        d+os.sep+filename) != 0

    dirs_with_veritracer_file = list(filter(isfile(args.filename), dirs))
    dirs_without_veritracer_file = list(filter(isnotfile(args.filename), dirs))
    dirs_with_empty_veritracer_file = list(filter(isemptyfile(args.filename),
                                                  dirs_with_veritracer_file))
    dirs_with_notempty_veritracer_file = list(filter(isnotemptyfile(args.filename),
                                                     dirs_with_veritracer_file))

    dirs_with_backtrace_file = list(
        filter(isfile(args.backtrace_filename), dirs))
    dirs_without_backtrace_file = list(filter(
        isnotfile(args.backtrace_filename), dirs))
    dirs_with_empty_backtrace_file = list(filter(isemptyfile(args.backtrace_filename),
    ...
    if dirs_without_veritracer_file != []:
        ...
        print(
            f"Warning: directories do not have {args.filename} files {empty_list}")

    if dirs_with_empty_veritracer_file != []:
        empty_list = pretty_printer_list(dirs_with_empty_veritracer_file)
        print(
            f"Warning: {args.filename} files are empty in these directories {empty_list}")

    if dirs_without_backtrace_file != []:
        empty_list = pretty_printer_list(dirs_without_backtrace_file)
        print(
            f"Warning: directories do not have {args.backtrace_filename} files {empty_list}")

    if dirs_with_empty_backtrace_file != []:
        empty_list = pretty_printer_list(dirs_with_empty_backtrace_file)
        print(
            f"Warning: {args.backtrace_filename} files are empty in these directories {empty_list}")
...
def partition_samples(args):
    ...
    while dir_bt_dict != {}:
        ...
        dirs_list = [dir_]
        dirs_with_same_bt_size_list = list(filter(
            lambda btl: len(btl[1]) == len(bt_list), dict_vi))
    ...
    backtrace_list_reduced_filename = backtrace_dirname + \
        os.sep + backtrace_reduced_map_filename
