...
def init_module(subparsers, veritracer_plugins):
    ...
    launch_parser = subparsers.add_parser(
        "launch", help="Launch parallel executions of binary")
    ...
    launch_parser.add_argument(
        '--timeout', action="store", type=float, default=31536000)
...
def pretty_printer_error(error):
    if error.errno == errno.EEXIST:
        print(f"Error: cannot create directory {error.filename}: File exists")
        print(f"       Use --force option to override it")
    else:
