...
ValueTuple = namedtuple(
    'ValueTuple', ['sdn', 'mean', 'std', 'max', 'min', 'time'])


def init_module(subparsers, veritracer_plugins):
    veritracer_plugins["plot"] = run
    plot_parser = subparsers.add_parser(
        "plot", help="Plot significant number of variables over time")
...
def read_csv(args):
    ...
    if not os.path.isfile(args.filename):
        print(f"Inexisting file: {args.filename}")
        exit(1)
    ...
    if not has_header:
        ...
        print(f"CSV header not found, default header {dictReader.fieldnames}")
    else:
        csv_file.seek(0)
    ...
    if args.variables != None:
        dictReaderCleaned = filter(lambda row: is_valid_row(
            variables_to_plot, row), dictReader)
    else:
...
def get_backtrace(args):
    ...
    return list(map(str.split, backtrace_file))
...
def partition_signed_values(time, mean):
    ...
    ziptm = zip(time, mean)
    pos = filter(lambda x: float(x[1]) > 0, ziptm)
    neg = filter(lambda x: float(x[1]) <= 0, ziptm)
    neg = map(lambda x: (x[0], -float(x[1])), neg)
    tpos, mpos, tneg, mneg = [], [], [], []
    if pos != []:
        tpos, mpos = zip(*pos)
    if neg != []:
...
def get_colors_bt(args):
    ...
    if args.variables is None or len(args.variables) != 1:
    ...
    backtrace_filtered = list(filter(lambda x: int(
        x[0]) == int(args.variables[0]), backtrace))
    hash_value_list, backtrace_list, time_list = list(zip(*backtrace_filtered))
    ...
    colors_list = [map_backtrace_to_color[backtrace]
...
def fast_scatter(ax, x, y, alpha, legends_name, legend_name, backtrace_set, map_backtrace_to_color, colors_list):
    ...
    z = list(zip(x, y, colors_list))
    labels = []
    i = 0
    for bt in backtrace_set:
        color = map_backtrace_to_color[bt]
        tmp = list(filter(lambda xyc: all(xyc[2] == color), z))
        xx, yy, cc = zip(*tmp)
...
def get_name_dict(args):
    ...
    if args.location_info_map:
        ...
        location_info_map = os.getenv(
            'VERITRACER_LOCINFO_PATH') + os.path.sep + "locationInfo.map"
    else:
        location_info_map = "locationInfo.map"

    if not os.path.isfile(location_info_map):
        print(
            f"Warning: Cannot find --location-info-map={location_info_map} file")
        print(f"         locationInfo.map path can be export with VERITRACER_LOCINFO_PATH")
        return name_dict
    ...
    try:
        ...
        print(f"{str(err)} for line: {line}")
        exit(1)
    except OSError as err:
        print(f"Error: {err.strerror}: {err.filename}")
...
def plot_significant_number(values_dict, args):
    ...
    title = ax1.set_title('Minimal Virtual Precision evolution',
                          loc="center", size=args.font_size)
    ...
    if args.backtrace:
        backtrace_set, map_backtrace_to_color, colors_list = get_colors_bt(
            args)

    name_dict = get_name_dict(args)

    # for variable, stats_values_list in d_values.iteritems():
    for variable, stats_values_list in values_dict.items():
        ...
        values_list_sorted = sorted(
            stats_values_list, key=lambda value: value.time)

        sdn_list = list(map(lambda value: value.sdn, values_list_sorted))
        time_list = list(map(lambda value: value.time, values_list_sorted))

        if args.base:
            snd_list = list(map(lambda s: float(
                s) * math.log(10, args.base), sdn_list))

        if args.normalize_time:
            global time_start
            shift = time_start
            time_list = list(map(lambda i: i-shift, time_list))

        if args.invocation_mode:
            time_list = range(len(time_list))
        ...
        if args.mean or args.mean_only:
            ...
            plot_mean(ax2, time_list, mean_list, legend_name,
                      legends_name, labels, colors[color_i])
        if args.std or args.std_only:
            std_list = map(lambda value: value.std, values_list_sorted)
            plot_std(ax2, time_list, std_list, legend_name,
                     legends_name, labels, colors[color_i])
        if args.min_max:
