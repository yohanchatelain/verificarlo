...
import veritracer.backtrace as vtr_backtrace
import veritracer.stats as vtr_math
import veritracer.format.binary as fmtbinary
import veritracer.format.text as fmttext
import veritracer.format as vtr_fmt

csv_header = ["hash", "type", "time", "max", "min",
              "median", "mean", "std", "significant_digit_number"]
...
def init_module(subparsers, veritracer_plugins):
    ...
    analyze_parser = subparsers.add_parser(
        "analyze", help="Gathering values from several veritracer executions")
...
def parse_exp(exp):
    format = list(map(lambda valueLine: valueLine.format, exp))
    time = list(map(lambda valueLine: valueLine.time, exp))
    address = list(map(lambda valueLine: valueLine.address, exp))
    hashv = list(map(lambda valueLine: valueLine.hash, exp))
    value = list(map(lambda valueLine: valueLine.value, exp))
...
def get_files():
    ...
    list_directories = list(filter(os.path.isdir, list_dir))
    list_files = list(
        map(lambda d: os.path.join(d, filename), list_directories))
    return list(filter(os.path.isfile, list_files))
...
def compute_stats(valuesLine_list):
    ...
    list_values = list(map(lambda valueLine: valueLine.value, valuesLine_list))

    if data_checking:
        hash_value = set(
            map(lambda valueLine: valueLine.hash, valuesLine_list))
        sizeof_value = set(
            map(lambda valueLine: valueLine.format, valuesLine_list))
        assert(len(sizeof_value) == 1)
        assert(len(hash_value) == 1)

    mean_ = vtr_math.mean(list_values)
    std_ = vtr_math.std(list_values)
        ...
        for unit in mem_units:
            if read_bytes.find(unit) != -1:
                ...
                if B.isdigit():
                    ...
                    print(f"Unknown size: {read_bytes}")
                    exit(1)
...
def analyze_files_list(jobs_pool, files_list, offset):
    ...
    exp_list = list(zip(*exp_list_transposed))
...
def concat_partial_files(output_filename, nb_slices):
    ...
    for slc in range(nb_slices):
        output_filename_slice = "{output}.{slc}".format(
            output=output_filename, slc=slc)
...
def set_format_file(args):
    ...
    if args.format == "binary":
        ...
        print(f"Unknown format file: {args.format}")
        exit(1)
...
def run(args):
    ...
    if args.prefix_dir and os.path.isdir(args.prefix_dir):
        ...
        print(f"Unknown directory {args.prefix_dir}")
        return False
    ...
    try:
        ...
        print(f"Error: backtrace file {bt_error.message} does not exist")
        return False
    except vtr_backtrace.AllEmptyFile as empty_error:
        print(f"Error: all {empty_error.message} files are empty")
        return False

    jobs_pool = mp.Pool()

    for bt_name, dir_list in dict_bt_files.items():

        if args.verbose:
            print(bt_name, dir_list)

        files_list = list(map(lambda d: d + os.sep + args.filename, dir_list))
        ...
        output_filename = "{output}.{bt}".format(
            output=args.output, bt=bt_name)
