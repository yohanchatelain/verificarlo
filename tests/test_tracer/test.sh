...
for FP_TYPE in "DOUBLE" "FLOAT"; do
    ...
    for FMT in "binary" "text"; do
        ...
        echo " verificarlo -DNITER=${ITER} -D ${FP_TYPE} --include-file=$FUNCTION_FILE --tracer --tracer-format=${FMT} --tracer-backtrace  ${SOURCE_FILE}" \
        " -o muller_${FP_TYPE}_${FMT}_${LANGUAGE} -lm --verbose  --tracer-debug-mode --tracer-level=temporary"
        
        verificarlo -DNITER=${ITER} -D ${FP_TYPE} --include-file=${FUNCTION_FILE} --tracer --tracer-format=${FMT} --tracer-backtrace  ${SOURCE_FILE} \
        -o muller_${FP_TYPE}_${FMT}_${LANGUAGE} -lm --verbose --tracer-debug-mode --tracer-level=temporary
