...
verificarlo-c --coverage unst_branch.c -o unst_branch
...
gcov_report() {
  ...
  $LLVM_BINDIR/llvm-cov gcov -f -b unst_branch.c.1.gcda > /dev/null
}
...
if diff -u unst_branch.c.gcov.IEEE unst_branch.c.gcov.RR53; then
  echo
