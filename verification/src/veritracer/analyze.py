...
from veritracer import backtrace
from veritracer import stats 
from veritracer.format import binary 
from veritracer.format import text
...
def parse_file(filename, offset):
    if format_file == "binary":
        return format.binary.parse_file(filename, offset)
    elif format_file == "text":
        return format.text.parse_file(filename, offset)
    else:
        assert(0)
...
def parse_exp(exp):
    ...
    valueline = format.format.ValuesLine(format=format[0],
                                         time=time[0],
                                         address=address[0],
                                         hash=hashv[0],
                                         value=value)
...
def compute_stats(valuesLine_list):
    ...
    mean_   = stats.mean(list_values)
    std_    = stats.std(mean_, list_values)
    median_ = stats.median(list_values)
    max_    = max(list_values)
    min_    = min(list_values)    
    sdn_    = stats.sdn(mean_, std_, sizeof_value)
...
def run(args):
    ...
    try:
        dict_bt_files = backtrace.partition_samples(args)
    except backtrace.BacktraceFileNoExist as bt_error:
        print("Error: backtrace file {bt} does not exist".format(bt=bt_error.message))
        return False
    except backtrace.AllEmptyFile as empty_error:
    ...
    for bt_name, dir_list in dict_bt_files.iteritems():
        ...
        if args.verbose:
            print("{name} {dir}".format(name=bt_name, dir=dir_list))
