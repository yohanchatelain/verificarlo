...
def read_csv(args):
    ...
    if not os.path.isfile(args.filename):
        print("Inexisting file: {filename}".format(filename=args.filename))
        exit(1)
    ...
    if not has_header:        
        ...
        print("CSV header not found, default header {fields}".format(fields=dictReader.fieldnames))
    else:
        csv_file.seek(0)
...
def partition_signed_values(time, mean):
    ...
    neg = map(lambda x : (t,-float(x[1])), neg)
    tpos,mpos,tneg,mneg = [],[],[],[]
    if pos != []:
        tpos,mpos = zip(*pos)
    if neg != []:
...
def get_colors_bt(args):
    ...
    # triple = (hash value, backtrace, time stamp)
    backtrace_filtered = filter(lambda triple: int(triple[0]) == args.variables[0], backtrace)
...
def fast_scatter(ax,x,y,alpha,legends_name,legend_name,backtrace_set,map_backtrace_to_color,colors_list):
    ...
    for bt in backtrace_set:
        ...
        # triple = (_,_,color)
        tmp = filter(lambda triple: all(triple[2]==color), z)
        xx,yy,cc = zip(*tmp)
...
def get_name_dict(args):
    ...
    try:
        ...
        print("{err} for line {line}".format(err=err, line=line))
        exit(1)
