...
BacktraceCallLine = namedtuple('BacktraceCallLine',
                               ['backtrace_hash', 'variable_hash', 'call_count'])

def parse_raw_backtrace_line(raw_line):
    parsed_value = BacktraceCallLine(backtrace_hash=raw_line[0],
                                     variable_hash=raw_line[1],
                                     call_count=raw_line[2])
    return parsed_value

# Size in bytes of backtrace line
# Backtrace_Hash Variable_Hash Count
# uint32_t       uint64_t      uint64_t
sizeof_backtrace_line_struct = 4 + 8 + 8 

def parse_backtrace(backtrace_filename):
    backtrace_file_input = open(backtrace_filename, 'rb')

    read_line = lambda : backtrace_file_input.read(sizeof_backtrace_line_struct)
    
    bytes_read = read_line()
    values_list = []
    append = values_list.append

    while bytes_read != '':
        raw_values = struct.unpack('=IQQ', bytes_read)
        values = parse_raw_backtrace_line(raw_values)
        append(values)
        bytes_read = read_line()
...
def dump_backtrace_list(bt_filename, backtrace_list):
    bt_file = open(bt_filename, "w")
    for backtrace in backtrace_list:
        output = "{bt_hash} {var_hash} {call_count}\n".format(
            bt_hash=backtrace.backtrace_hash,
            var_hash=backtrace.variable_hash,
            call_count=backtrace.call_count)
        bt_file.write(output)
    bt_file.close()
...
def check_divergence(bt_list, counter):
    ...
    for i in range(minlen):
        ...
        print("{line} {is_good}".format(line=line,is_good=is_good_line))
        if  not is_good_line:
...
def partition_samples(args):
    ...
    while dir_bt_dict != {}:
        ...
        dirs_list = [dir_]
        dirs_with_same_bt_size_list = filter(lambda btl : len(btl[1]) == len(bt_list), dict_vi)
