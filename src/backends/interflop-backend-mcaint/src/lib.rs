...
#[repr(C)]
#[derive(Debug, Copy, Clone, PartialEq, Eq)]
pub enum mcaint_err_mode {
    mcaint_err_mode_rel = 0,
    mcaint_err_mode_abs,
    mcaint_err_mode_all,
...
#[repr(C)]
pub struct mcaint_context_t {
    pub seed: u64,
    pub binary32_precision: c_int,
    pub binary64_precision: c_int,
    pub absErr_exp: c_int,
    pub mode: mcaint_mode,
    pub relErr: bool,
    pub absErr: bool,
...
static BACKEND_NAME: &[u8] = b"interflop-mcaint\0";
...
#[no_mangle]
pub extern "C" fn interflop_mca_int_get_backend_name() -> *const c_char {
    BACKEND_NAME.as_ptr() as *const c_char
}

#[no_mangle]
pub extern "C" fn interflop_mca_int_get_backend_version() -> *const c_char {
...
unsafe fn noise_f64(x: &mut f64, exp: i32, ctx: &mcaint_context_t) {
    if ctx.mode == mcaint_mode::mcaint_mode_ieee || !x.is_finite() || *x == 0.0 {
        return;
    }
...
unsafe fn noise_f32(x: &mut f32, exp: i32, ctx: &mcaint_context_t) {
    if ctx.mode == mcaint_mode::mcaint_mode_ieee || !x.is_finite() || *x == 0.0 {
        return;
    }
    let mut d = *x as f64;
...
#[no_mangle]
pub unsafe extern "C" fn interflop_mca_int_add_float(
    a: f32,
    b: f32,
    c: *mut f32,
    context: *mut c_void,
    ...
    *c = res;
    if !context.is_null() {
        let ctx = &*(context as *const mcaint_context_t);
...
#[no_mangle]
pub unsafe extern "C" fn interflop_mca_int_sub_float(
    a: f32,
    b: f32,
    c: *mut f32,
    context: *mut c_void,
...
#[no_mangle]
pub unsafe extern "C" fn interflop_mca_int_mul_float(
    a: f32,
    b: f32,
    c: *mut f32,
    context: *mut c_void,
...
#[no_mangle]
pub unsafe extern "C" fn interflop_mca_int_div_float(
    a: f32,
    b: f32,
    c: *mut f32,
    context: *mut c_void,
...
#[no_mangle]
pub unsafe extern "C" fn interflop_mca_int_add_double(
    a: f64,
    b: f64,
    c: *mut f64,
    context: *mut c_void,
...
#[no_mangle]
pub unsafe extern "C" fn interflop_mca_int_sub_double(
    a: f64,
    b: f64,
    c: *mut f64,
    context: *mut c_void,
...
#[no_mangle]
pub unsafe extern "C" fn interflop_mca_int_mul_double(
    a: f64,
    b: f64,
    c: *mut f64,
    context: *mut c_void,
...
#[no_mangle]
pub unsafe extern "C" fn interflop_mca_int_div_double(
    a: f64,
    b: f64,
    c: *mut f64,
    context: *mut c_void,
...
#[no_mangle]
pub unsafe extern "C" fn interflop_mca_int_pre_init(
    ...
    if !context.is_null() {
        let ctx = Box::new(mcaint_context_t {
            seed: 0,
            binary32_precision: 24,
            binary64_precision: 53,
            absErr_exp: -100,
            mode: mcaint_mode::mcaint_mode_mca,
            relErr: true,
            absErr: false,
            choose_seed: false,
            daz: false,
...
#[no_mangle]
pub unsafe extern "C" fn interflop_mca_int_cli(
    argc: c_int,
    argv: *mut *mut c_char,
    context: *mut c_void,
) {
    ...
    let ctx = &mut *(context as *mut mcaint_context_t);
    for i in 1..argc as usize {
        ...
        if let Ok(arg) = CStr::from_ptr(ptr).to_str() {
            if arg.starts_with("--precision-binary32=") {
                ...
                match &arg["--mode=".len()..] {
                    "ieee" => ctx.mode = mcaint_mode::mcaint_mode_ieee,
                    "mca" => ctx.mode = mcaint_mode::mcaint_mode_mca,
                    "pb" => ctx.mode = mcaint_mode::mcaint_mode_pb,
                    "rr" => ctx.mode = mcaint_mode::mcaint_mode_rr,
                    _ => {}
                }
...
#[no_mangle]
pub unsafe extern "C" fn interflop_mca_int_configure(configure: *mut c_void, context: *mut c_void) {
    if !configure.is_null() && !context.is_null() {
        let conf = &*(configure as *const mcaint_context_t);
        let ctx = &mut *(context as *mut mcaint_context_t);
        ctx.seed = conf.seed;
        ctx.binary32_precision = conf.binary32_precision;
        ctx.binary64_precision = conf.binary64_precision;
        ctx.absErr_exp = conf.absErr_exp;
        ctx.mode = conf.mode;
        ctx.relErr = conf.relErr;
        ctx.absErr = conf.absErr;
...
#[no_mangle]
pub unsafe extern "C" fn interflop_mca_int_init(
    _context: *mut c_void,
) -> interflop_backend_interface_t {
    interflop_backend_interface_t {
        interflop_add_float: Some(interflop_mca_int_add_float),
        interflop_sub_float: Some(interflop_mca_int_sub_float),
        interflop_mul_float: Some(interflop_mca_int_mul_float),
        interflop_div_float: Some(interflop_mca_int_div_float),
        interflop_cmp_float: None,
        interflop_add_double: Some(interflop_mca_int_add_double),
        interflop_sub_double: Some(interflop_mca_int_sub_double),
        interflop_mul_double: Some(interflop_mca_int_mul_double),
        interflop_div_double: Some(interflop_mca_int_div_double),
...
pub unsafe extern "C" fn interflop_init(context: *mut c_void) -> interflop_backend_interface_t {
    interflop_mca_int_init(context)
}
