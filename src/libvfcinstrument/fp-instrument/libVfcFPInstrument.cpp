...
static cl::opt<std::string> VfclibFPInstFunction(
    "vfclibfpinst-function",
    cl::desc("Only instrument fp operations of given FunctionName"),
    cl::value_desc("FunctionName"), cl::init(""));

static cl::opt<std::string> VfclibFPInstIncludeFile(
    "vfclibfpinst-include-file",
    cl::desc(
        "Only instrument fp of modules / functions in file IncludeNameFile "),
    cl::value_desc("IncludeNameFile"), cl::init(""));

static cl::opt<std::string> VfclibFPInstExcludeFile(
    "vfclibfpinst-exclude-file",
    cl::desc(
        "Do not instrument fp of modules / functions in file ExcludeNameFile "),
    cl::value_desc("ExcludeNameFile"), cl::init(""));

static cl::opt<bool> VfclibFPInstInstrumentFCMP(
    "vfclibfpinst-inst-fcmp", cl::desc("Instrument floating point comparisons"),
...
std::string Fops2str[] = {"add", "sub",          "mul",   "div",
                          "cmp", "call_handler", "ignore"};

struct VfclibFPInst : public ModulePass {
  static char ID;

  std::set<std::string> IncludedFunctionSet;
  std::set<std::string> ExcludedFunctionSet;

  VfclibFPInst() : ModulePass(ID) {}
  ...
  bool runOnModule(Module &M) {
    ...
    parseFunctionSetFile(M, VfclibFPInstIncludeFile, IncludedFunctionSet);
    parseFunctionSetFile(M, VfclibFPInstExcludeFile, ExcludedFunctionSet);

    // Parse instrument single function option (--function)
    if (!VfclibFPInstFunction.empty()) {
      IncludedFunctionSet.insert(VfclibFPInstFunction);
    ...
    for (Module::iterator F = M.begin(), E = M.end(); F != E; ++F) {
      ...
      if (VfclibFPInstExcludeFile.empty() && !VfclibFPInstIncludeFile.empty()) {
        continue;
      } else {
...
char VfclibFPInst::ID = 0;
static RegisterPass<VfclibFPInst>
    X("vfclibfpinst", "verificarlo fp instrument pass", false, false);
