...
//
// Copyright (C) 2017
//     Universite de Versailles St-Quentin-en-Yvelines * CMLA, Ecole Normale
...
static int _set_mca_precision(int precision) {
  MCALIB_T = precision;
  float_bitmask =
      (MCALIB_T <= FLOAT_PREC) ? (-1) << (FLOAT_PREC - MCALIB_T) : FLOAT_MASK_1;
  double_bitmask = (MCALIB_T <= DOUBLE_PREC)
                       ? (-1ULL) << (DOUBLE_PREC - MCALIB_T)
                       : DOUBLE_MASK_1;
  return 0;
}
...
static float _bitmask_sbin(float a, float b, const int op) {
  ...
  if (BITMASKLIB_MODE == BITMASK_MODE_RAND) {
    ...
    /* Remove the higher bits for restoring the bits saved by applying an OR
     * bitmask */
