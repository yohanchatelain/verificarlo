...
void _fmaxN(const T *HWY_RESTRICT a, const T *HWY_RESTRICT b,
            const T *HWY_RESTRICT c, T *HWY_RESTRICT r) {
  const D d;
  const auto va = hn::Load(d, a);
  const auto vb = hn::Load(d, b);
...
#define define_fp_xN_tern_op(type, name, size, op)                             \
...
define_fp_xN_tern_op(float, f32, 1, fma);
#endif
...
define_fp_xN_tern_op(double, f64, 1, fma);
...
define_fp_xN_tern_op(float, f32, 2, fma);
#endif
...
define_fp_xN_tern_op(double, f64, 2, fma);
...
define_fp_xN_tern_op(float, f32, 4, fma);
#endif
...
define_fp_xN_tern_op(double, f64, 4, fma);
...
define_fp_xN_tern_op(float, f32, 8, fma);
#endif
...
define_fp_xN_tern_op(double, f64, 8, fma);
...
define_fp_xN_tern_op(float, f32, 16, fma);
#endif
template <typename T>
...
void _fma_f32(const float *HWY_RESTRICT a, const float *HWY_RESTRICT b,
              const float *HWY_RESTRICT c, float *HWY_RESTRICT d,
              const size_t count) {
  _fma<float>(a, b, c, d, count);
}
...
void _fma_f64(const double *HWY_RESTRICT a, const double *HWY_RESTRICT b,
              const double *HWY_RESTRICT c, double *HWY_RESTRICT d,
              const size_t count) {
  _fma<double>(a, b, c, d, count);
}
...
void fma(const T *HWY_RESTRICT a, const T *HWY_RESTRICT b,
         const T *HWY_RESTRICT c, T *HWY_RESTRICT d, const size_t count) {
  if constexpr (std::is_same_v<T, float>) {
    return HWY_DYNAMIC_DISPATCH(_fma_f32)(a, b, c, d, count);
  } else if constexpr (std::is_same_v<T, double>) {
    return HWY_DYNAMIC_DISPATCH(_fma_f64)(a, b, c, d, count);
  }
}
...
void fmaf32x2(const float *HWY_RESTRICT a, const float *HWY_RESTRICT b,
              const float *HWY_RESTRICT c, float *HWY_RESTRICT d) {
  HWY_DYNAMIC_DISPATCH(_fmax2_f32)(a, b, c, d);
}
#endif
...
void fmaf64x2(const double *HWY_RESTRICT a, const double *HWY_RESTRICT b,
              const double *HWY_RESTRICT c, double *HWY_RESTRICT d) {
  HWY_DYNAMIC_DISPATCH(_fmax2_f64)(a, b, c, d);
}
...
void fmaf32x4(const float *HWY_RESTRICT a, const float *HWY_RESTRICT b,
              const float *HWY_RESTRICT c, float *HWY_RESTRICT d) {
  HWY_DYNAMIC_DISPATCH(_fmax4_f32)(a, b, c, d);
}
...
void fmaf32x8(const float *HWY_RESTRICT a, const float *HWY_RESTRICT b,
              const float *HWY_RESTRICT c, float *HWY_RESTRICT d) {
  HWY_DYNAMIC_DISPATCH(_fmax8_f32)(a, b, c, d);
}
...
void fmaf64x4(const double *HWY_RESTRICT a, const double *HWY_RESTRICT b,
              const double *HWY_RESTRICT c, double *HWY_RESTRICT d) {
  HWY_DYNAMIC_DISPATCH(_fmax4_f64)(a, b, c, d);
}
...
void fmaf64x8(const double *HWY_RESTRICT a, const double *HWY_RESTRICT b,
              const double *HWY_RESTRICT c, double *HWY_RESTRICT d) {
  HWY_DYNAMIC_DISPATCH(_fmax8_f64)(a, b, c, d);
}
...
void fmaf32x16(const float *HWY_RESTRICT a, const float *HWY_RESTRICT b,
               const float *HWY_RESTRICT c, float *HWY_RESTRICT d) {
  HWY_DYNAMIC_DISPATCH(_fmax16_f32)(a, b, c, d);
}
...
void fmaf64x16(const double *HWY_RESTRICT a, const double *HWY_RESTRICT b,
               const double *HWY_RESTRICT c, double *HWY_RESTRICT d) {
  HWY_DYNAMIC_DISPATCH(_fmax16_f64)(a, b, c, d);
}
...
// TODO: Add guard to check if compiled for X86_64
// use the dynamic dispatch function directly since f32x2, f32x4 and f32x2
// are natively natively on x86_64 architectures (-march=x86_64 which is the
// default) and will never be pointers.

f32x2_v addf32x2_d(const f32x2_v a, const f32x2_v b) {
  const float *a_ptr = reinterpret_cast<const float *>(&a);
  const float *b_ptr = reinterpret_cast<const float *>(&b);
  alignas(8) float result_ptr[2];
  HWY_DYNAMIC_DISPATCH(_addx2_f32)(a_ptr, b_ptr, result_ptr);
  f32x2_v result;
  std::memcpy(&result, result_ptr, sizeof(f32x2_v));
  return result;
}
...
f32x2_v subf32x2_d(const f32x2_v a, const f32x2_v b) {
  ...
  const float *b_ptr = reinterpret_cast<const float *>(&b);
  alignas(8) float result_ptr[2];
  HWY_DYNAMIC_DISPATCH(_subx2_f32)(a_ptr, b_ptr, result_ptr);
  f32x2_v result;
  std::memcpy(&result, result_ptr, sizeof(f32x2_v));
  return result;
}
...
f32x2_v mulf32x2_d(const f32x2_v a, const f32x2_v b) {
  ...
  const float *b_ptr = reinterpret_cast<const float *>(&b);
  alignas(8) float result_ptr[2];
  HWY_DYNAMIC_DISPATCH(_mulx2_f32)(a_ptr, b_ptr, result_ptr);
  f32x2_v result;
  std::memcpy(&result, result_ptr, sizeof(f32x2_v));
  return result;
}
...
f32x2_v divf32x2_d(const f32x2_v a, const f32x2_v b) {
  ...
  const float *b_ptr = reinterpret_cast<const float *>(&b);
  alignas(8) float result_ptr[2];
  HWY_DYNAMIC_DISPATCH(_divx2_f32)(a_ptr, b_ptr, result_ptr);
  f32x2_v result;
  std::memcpy(&result, result_ptr, sizeof(f32x2_v));
  return result;
}
...
f32x2_v sqrtf32x2_d(const f32x2_v a) {
  const float *a_ptr = reinterpret_cast<const float *>(&a);
  alignas(8) float result_ptr[2];
  HWY_DYNAMIC_DISPATCH(_sqrtx2_f32)(a_ptr, result_ptr);
  f32x2_v result;
  std::memcpy(&result, result_ptr, sizeof(f32x2_v));
  return result;
}
...
f32x2_v fmaf32x2_d(const f32x2_v a, const f32x2_v b, const f32x2_v c) {
  ...
  const float *c_ptr = reinterpret_cast<const float *>(&c);
  alignas(8) float result_ptr[2];
  HWY_DYNAMIC_DISPATCH(_fmax2_f32)(a_ptr, b_ptr, c_ptr, result_ptr);
  f32x2_v result;
  std::memcpy(&result, result_ptr, sizeof(f32x2_v));
  return result;
}
...
f64x2_v addf64x2_d(const f64x2_v a, const f64x2_v b) {
  ...
  const double *b_ptr = reinterpret_cast<const double *>(&b);
  alignas(16) double result_ptr[2];
  HWY_DYNAMIC_DISPATCH(_addx2_f64)(a_ptr, b_ptr, result_ptr);
  f64x2_v result;
  std::memcpy(&result, result_ptr, sizeof(f64x2_v));
  return result;
}
...
f64x2_v subf64x2_d(const f64x2_v a, const f64x2_v b) {
  ...
  const double *b_ptr = reinterpret_cast<const double *>(&b);
  alignas(16) double result_ptr[2];
  HWY_DYNAMIC_DISPATCH(_subx2_f64)(a_ptr, b_ptr, result_ptr);
  f64x2_v result;
  std::memcpy(&result, result_ptr, sizeof(f64x2_v));
  return result;
}
...
f64x2_v mulf64x2_d(const f64x2_v a, const f64x2_v b) {
  ...
  const double *b_ptr = reinterpret_cast<const double *>(&b);
  alignas(16) double result_ptr[2];
  HWY_DYNAMIC_DISPATCH(_mulx2_f64)(a_ptr, b_ptr, result_ptr);
  f64x2_v result;
  std::memcpy(&result, result_ptr, sizeof(f64x2_v));
  return result;
}
...
f64x2_v divf64x2_d(const f64x2_v a, const f64x2_v b) {
  ...
  const double *b_ptr = reinterpret_cast<const double *>(&b);
  alignas(16) double result_ptr[2];
  HWY_DYNAMIC_DISPATCH(_divx2_f64)(a_ptr, b_ptr, result_ptr);
  f64x2_v result;
  std::memcpy(&result, result_ptr, sizeof(f64x2_v));
  return result;
}
...
f64x2_v sqrtf64x2_d(const f64x2_v a) {
  const double *a_ptr = reinterpret_cast<const double *>(&a);
  alignas(16) double result_ptr[2];
  HWY_DYNAMIC_DISPATCH(_sqrtx2_f64)(a_ptr, result_ptr);
  f64x2_v result;
  std::memcpy(&result, result_ptr, sizeof(f64x2_v));
  return result;
}
...
f64x2_v fmaf64x2_d(const f64x2_v a, const f64x2_v b, const f64x2_v c) {
  ...
  const double *c_ptr = reinterpret_cast<const double *>(&c);
  alignas(16) double result_ptr[2];
  HWY_DYNAMIC_DISPATCH(_fmax2_f64)(a_ptr, b_ptr, c_ptr, result_ptr);
  f64x2_v result;
  std::memcpy(&result, result_ptr, sizeof(f64x2_v));
  return result;
}
...
f32x4_v addf32x4_d(const f32x4_v a, const f32x4_v b) {
  ...
  const float *b_ptr = reinterpret_cast<const float *>(&b);
  alignas(16) float result_ptr[4];
  HWY_DYNAMIC_DISPATCH(_addx4_f32)(a_ptr, b_ptr, result_ptr);
  f32x4_v result;
  std::memcpy(&result, result_ptr, sizeof(f32x4_v));
  return result;
}
...
f32x4_v subf32x4_d(const f32x4_v a, const f32x4_v b) {
  ...
  const float *b_ptr = reinterpret_cast<const float *>(&b);
  alignas(16) float result_ptr[4];
  HWY_DYNAMIC_DISPATCH(_subx4_f32)(a_ptr, b_ptr, result_ptr);
  f32x4_v result;
  std::memcpy(&result, result_ptr, sizeof(f32x4_v));
  return result;
}
...
f32x4_v mulf32x4_d(const f32x4_v a, const f32x4_v b) {
  ...
  const float *b_ptr = reinterpret_cast<const float *>(&b);
  alignas(16) float result_ptr[4];
  HWY_DYNAMIC_DISPATCH(_mulx4_f32)(a_ptr, b_ptr, result_ptr);
  f32x4_v result;
  std::memcpy(&result, result_ptr, sizeof(f32x4_v));
  return result;
}
...
f32x4_v divf32x4_d(const f32x4_v a, const f32x4_v b) {
  ...
  const float *b_ptr = reinterpret_cast<const float *>(&b);
  alignas(16) float result_ptr[4];
  HWY_DYNAMIC_DISPATCH(_divx4_f32)(a_ptr, b_ptr, result_ptr);
  f32x4_v result;
  std::memcpy(&result, result_ptr, sizeof(f32x4_v));
  return result;
}
...
f32x4_v sqrtf32x4_d(const f32x4_v a) {
  const float *a_ptr = reinterpret_cast<const float *>(&a);
  alignas(16) float result_ptr[4];
  HWY_DYNAMIC_DISPATCH(_sqrtx4_f32)(a_ptr, result_ptr);
  f32x4_v result;
  std::memcpy(&result, result_ptr, sizeof(f32x4_v));
  return result;
}
...
f32x4_v fmaf32x4_d(const f32x4_v a, const f32x4_v b, const f32x4_v c) {
  ...
  const float *c_ptr = reinterpret_cast<const float *>(&c);
  alignas(16) float result_ptr[4];
  HWY_DYNAMIC_DISPATCH(_fmax4_f32)(a_ptr, b_ptr, c_ptr, result_ptr);
  f32x4_v result;
  std::memcpy(&result, result_ptr, sizeof(f32x4_v));
  return result;
}
