...
static inline void prng_gen(prng_state *s, uint8_t buf[], size_t size) {
  __m256i o0 = s->output[0], o1 = s->output[1], o2 = s->output[2],
          o3 = s->output[3], s0 = s->state[0], s1 = s->state[1],
          s2 = s->state[2], s3 = s->state[3], t0, t1, t2, t3, u0, u1, u2, u3,
          counter = s->counter;
...
#define STEPS 8
#define ROUNDS 13
#define SHISHUA_RNG_BUFFER_SIZE (STEPS * 128)
  uint8_t buf[SHISHUA_RNG_BUFFER_SIZE];
  ...
  s->state[3] =
      _mm256_set_epi64x(phi[15], phi[14] ^ seed[1], phi[13], phi[12] ^ seed[0]);
  for (size_t i = 0; i < ROUNDS; i++) {
    prng_gen(s, buf, SHISHUA_RNG_BUFFER_SIZE);
...
static inline void shishua_gen(prng_state *s, uint8_t buf[]) {
  prng_gen(s, buf, SHISHUA_RNG_BUFFER_SIZE);
}
